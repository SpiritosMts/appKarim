...
Future<List<DocumentSnapshot>> getDocumentsByCollCondition(
  ...
  List userStoresIDs = [];
  QuerySnapshot snap = (onlyAccepted ? await coll.where('accepted', isEqualTo: 'yes').get() : await coll.get());

  final List<DocumentSnapshot> documentsFound =
      snap.docs; //return QueryDocumentSnapshot .data() to convert to json// or "userDoc.get('email')" for each field
...
void deleteFromMap({coll, docID, fieldMapName, mapKeyToDelete, bool withBackDialog = false}) {
  coll.doc(docID).get().then((DocumentSnapshot documentSnapshot) async {
    if (documentSnapshot.exists) {
      Map<String, dynamic> fieldMap = documentSnapshot.get(fieldMapName);

      fieldMap.remove(mapKeyToDelete);
      ...
      await coll.doc(docID).update({
        ...
        print('## item from fieldMap deleted');
        //showSnack('appointment declined',color:Colors.black54);
      }).catchError((error) async {
        print('## item from fieldMap FAILED to deleted');
